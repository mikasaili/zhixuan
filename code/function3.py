...
def pie_degree(res, ax):
    d_degree = res[1]
    labels = ['Doctorate', 'Master', 'Bachelor', 'Associate', 'Diploma', 'Technical Secondary']
    ...
    ax.set_title("Current Recruiter Education Distribution")
    ax.pie(X, labels=labels, colors=colors, autopct='%1.2f%%', pctdistance=0.8)
...
def pie_age(res, ax):
    ...
    ax.set_title("Current Recruiter Age Distribution")
    ax.pie(X, labels=labels, colors=colors, autopct='%1.2f%%', pctdistance=0.8)
...
if __name__ == '__main__':
    path = 'F:\\CODE_Djiango\\dataset_CV\\CV\\'
